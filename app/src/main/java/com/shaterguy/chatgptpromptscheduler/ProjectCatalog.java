...
/** Local project catalog populated only by visited ChatGPT project pages and legacy schedules. */
final class ProjectCatalog {
    ...
    List<ProjectUrlPolicy.ProjectRef> entries() {
        ...
        out.sort(Comparator.comparing(this::displayName, String.CASE_INSENSITIVE_ORDER));
        if (raw == null || !urls.equals(raw)) prefs.edit().putStringSet(KEY_URLS, urls).commit();
        return out;
    }
    ...
    boolean addVisitedProject(String rawUrl, String displayName) {
        ...
        LinkedHashSet<String> urls = canonicalizeStoredUrls(prefs.getStringSet(KEY_URLS, Collections.emptySet()));
        boolean added = urls.add(ref.canonicalUrl);
        ...
        if (!added && !nameChanged) return false;
        ...
        if (!editor.commit()) throw new IllegalStateException("프로젝트 목록을 저장하지 못했습니다.");
        return true;
    }

    void seedFromSchedules(List<Schedule> schedules) {
        if (schedules == null) return;
        for (Schedule schedule : schedules) {
            if (schedule == null || !"project".equals(schedule.targetType)) continue;
            ProjectUrlPolicy.ProjectRef ref = ProjectUrlPolicy.parseProject(schedule.targetUrl);
            if (ref != null) addVisitedProject(ref.canonicalUrl, "");
        }
    }
    ...
    static String fallbackDisplayName(ProjectUrlPolicy.ProjectRef ref) {
        ...
        return "프로젝트 " + (id.length() > 20 ? id.substring(0, 20) : id);
    }

    private static String nameKey(String projectId) {
        return KEY_NAME_PREFIX + projectId;
    }
