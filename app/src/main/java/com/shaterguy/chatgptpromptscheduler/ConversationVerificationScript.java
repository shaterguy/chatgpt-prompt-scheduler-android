...
public final class ConversationVerificationScript {
    ...
    public static String build(Schedule schedule, String stampedPrompt) {
        ...
        return "(() => {" +
                ...
                "const persistentConversation=!!actualConversation&&!/^WEB:/i.test(actualConversation),persistentExpectedConversation=!!expectedConversation&&!/^WEB:/i.test(expectedConversation);" +
                "const homePath=location.pathname==='/'||location.pathname==='';" +
                "const routeDiagnostics={expectedType,expectedProject,expectedConversation,rawActualProject,actualProject,actualConversation,persistentConversation,userMessages:users.length,promptAlreadyPresent};" +
                "if(expectedType==='existing'){if(!persistentExpectedConversation||!persistentConversation||actualConversation!==expectedConversation||(expectedProject?actualProject!==expectedProject:!!actualProject))return result('TARGET_CONTEXT_MISMATCH','expected='+expectedUrl+' actual='+location.href,routeDiagnostics);}" +
                "else if(expectedType==='project'){if(!expectedProject||actualProject!==expectedProject)return result('TARGET_CONTEXT_MISMATCH','expected='+expectedUrl+' actual='+location.href,routeDiagnostics);if(!persistentConversation)return result('RETRY','실제 대화 주소 생성 대기',routeDiagnostics);}" +
                "else if(expectedType==='general'){if(actualProject)return result('TARGET_CONTEXT_MISMATCH','expected='+expectedUrl+' actual='+location.href,routeDiagnostics);if(!persistentConversation){if(homePath||!!actualConversation)return result('RETRY','실제 대화 주소 생성 대기',routeDiagnostics);return result('TARGET_CONTEXT_MISMATCH','expected='+expectedUrl+' actual='+location.href,routeDiagnostics);}}" +
