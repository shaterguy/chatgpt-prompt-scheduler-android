...
public final class OrchestrationScript {
    ...
    public static String prepare(String prompt) {
        ...
        return "(() => {" + common() +
                ...
                "const actual=read();if(actual&&actual!==expected){let hash=2166136261;for(let i=0;i<actual.length;i++){hash^=actual.charCodeAt(i);hash=Math.imul(hash,16777619);}return out('DRAFT_PRESENT','중계 대화 입력창에 다른 초안이 있습니다.',{draft_length:actual.length,draft_fingerprint:(hash>>>0).toString(16)});}" +
