...
현재 정식 버전은 `0.5.1`입니다. 일반 Chat, 프로젝트 새 대화, 기존 대화, Chat·Work 모드, 일회성·매일·매주·분 간격 반복, 기존 입력 교체, 설정 JSON 내보내기·가져오기와 실행 기록을 지원합니다.

## 0.5.1 실제 대화 주소 검증과 실행 기록 연결

예약 프롬프트를 보낸 뒤 단순히 목표 화면이 열린 것만으로 성공 처리하지 않고, 실제 `/c/...` 대화 주소가 생성되었는지와 대상 컨텍스트가 맞는지, 전송한 사용자 프롬프트가 해당 대화에 존재하는지를 함께 확인합니다. 일반 Chat 홈이나 프로젝트 루트처럼 아직 실제 대화 주소가 만들어지지 않은 상태는 완료로 보지 않고 계속 확인합니다.

검증이 완료되면 실제 대화 주소를 실행 기록의 `conversationUrl`에 저장합니다. 기존 실행 기록 형식은 계속 읽을 수 있으며, 실행 기록 화면은 확인된 대화 주소가 있을 때 해당 대화로 다시 접근할 수 있는 정보를 유지합니다.
