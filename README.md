...
v0.2.0은 애플리케이션 코드·리소스·테스트를 v0.1.15 기준으로 되돌린 정식 릴리스입니다. 현재 공개 정식본 v2100000000과 동일한 내부 versionCode 2100000000을 유지하고 사용자 표시 버전만 0.2.0으로 정리했습니다. Android 플랫폼은 동일 Application ID·동일 서명 인증서에서 새 APK의 versionCode가 설치본과 같거나 높으면 업데이트를 허용하므로, 공개판 고정 서명 인증서를 유지해 덮어쓰기 업데이트 경로를 보존합니다.

일반·프로젝트·기존 대화, Chat·Work 모드, 일회성·매일·매주·분 간격 반복, 기존 입력 교체, 설정 JSON 내보내기·가져오기, 스마트폰·태블릿 UI를 지원합니다.

기능 기준은 v0.1.15와 동일합니다. 오토런 중계의 감시·수신·전달·제출 확인 상태를 분리해 저장하고, 정상 응답은 시간 제한 없이 감시합니다. 제출 전후 프로세스가 종료되어도 DOM의 동일 사용자 턴을 확인해 중복 클릭을 막으며, 불명확한 결과와 규정 외 응답은 알림 후 안전하게 일시정지합니다. `WAITING_USER` 및 사용자 조치 재검증 흐름을 포함하며, 기존 예약 생성·저장·알림·실행 경로와 JSON 형식은 v0.1.15 기준입니다.
...
v0.1.2 이후 공개 APK와 동일한 고정 인증서를 사용합니다. v0.2.0 릴리스 워크플로는 v2100000000 설치본과 새 APK의 Application ID, 서명 인증서와 versionCode 동일성을 직접 검증합니다.
