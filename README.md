...
v0.1.20의 Protocol 3.3 오토런은 앱이 bootstrap 권한을 가집니다. Job ID를 자동 생성하고 지정 프로젝트에 실행 전용 일반 Chat을 만든 뒤, 첫 `AR_SEND_WORK` 때 같은 프로젝트에 Work를 지연 생성합니다. 두 대화의 실제 URL은 첫 사용자 턴과 프로젝트 경로를 함께 확인한 뒤 저장합니다. 기존 예약 생성·저장·알림·실행 경로와 JSON 형식은 변경하지 않았습니다.
...
`오토런 중계 열기`는 작업 이력 화면을 먼저 엽니다. 상단 `새 작업`에서 새 실행 설정으로 이동하며, 각 Job 카드에서 최근 수행 항목과 해당 Job만의 실행 로그(사용자용 요약) 및 디버그 로그(redacted JSONL)를 확인·저장할 수 있습니다. 작업별 디버그 로그는 앱 내부에서 별도 파일로 제한 보관합니다.
