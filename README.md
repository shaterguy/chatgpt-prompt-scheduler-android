...
v0.2.0은 애플리케이션 코드·리소스·테스트를 v0.1.15 기준으로 되돌린 정식 릴리스입니다. 현재 공개 정식본 v2100000000(versionCode 2100000000) 위에 덮어쓰기 업데이트할 수 있도록 내부 versionCode를 2100000001로 올렸고, 사용자 표시 버전은 0.2.0으로 정리했습니다. Application ID와 공개판 고정 서명 인증서는 유지합니다.
...
v0.1.2 이후 공개 APK와 동일한 고정 인증서를 사용합니다. v0.2.0은 v2100000000보다 높은 versionCode를 사용하므로 동일 공개판 설치본에서 덮어쓰기 업데이트할 수 있도록 릴리스 워크플로에서 직접 검증합니다.
