...
v0.1.18은 `새로 시작` 시 일반 Chat의 기존 입력 초안을 지우고 정확한 `[AUTOMATION_START ...]`를 항상 새 사용자 턴으로 전송합니다. 같은 conversation ID를 유지한 ChatGPT SPA 주소 정규화는 최초 시작을 막지 않습니다. 첫 일반 Chat 제어 신호는 기존 Drive Step/Round를 한 번 재설정할 수 있고, 이후에는 기존 중복 방지·순서 검증·보수적 대상 검사를 그대로 적용합니다. 기존 예약 생성·저장·알림·실행 경로와 JSON 형식은 변경하지 않았습니다.
...
4. 최초 시작 사용자 턴이 확인된 뒤에는 잘못된 Job·Step·Round, 중복 신호, 대상 URL 변경, 로그인 필요 또는 전송 결과 불명확 상태에서 자동 재전송을 하지 않고 확인 알림을 표시합니다.
