...
`오토런 중계 열기`는 작업 이력 화면을 먼저 엽니다. 상단 `새 작업`에서 새 실행 설정으로 이동하며, 모든 Job 카드는 `작업 열기`로 독립 보존된 상세 작업공간에 들어갈 수 있습니다. 상세 화면에는 해당 Job의 원본 요구사항, 프로젝트·대화 연결, 모델·추론, Step/Round, 마지막 신호·전달·오류와 해당 Job만의 실행 로그(사용자용 요약) 및 디버그 로그(redacted JSONL)가 표시됩니다. 검증된 `AR_DONE`만 성공 완료로 판정하며 `AR_PAUSE`와 `AR_ABORTED`는 각각 일시정지·중단 terminal로 구분합니다. terminal Job의 재개·일시정지·중지 버튼은 비활성화되고, 미완료 과거 Job은 전체 복구 스냅샷이 있을 때만 재개할 수 있습니다. 로그 버튼은 완료 여부와 무관하게 계속 동작합니다. `작업 삭제`는 확인/취소 대화상자를 거쳐 앱의 목록·상세 화면에서만 숨기며 ChatGPT 대화와 Drive 파일은 건드리지 않고 Job ID도 재사용하지 않습니다. 작업별 디버그 로그는 앱 내부에서 별도 파일로 제한 보관합니다.

사용자가 현재 Job을 수동으로 `일시정지`하면 그 Job의 전체 스냅샷을 저장하고 단일 중계 실행 슬롯을 양보하므로 새 Job을 시작할 수 있습니다. 일시정지된 Job의 `재개`는 계속 활성입니다. 다른 Job이 실행 중일 때 누르면 `취소`/`전환 및 재개` 확인창을 표시하고, 전환을 확인한 경우 현재 Job을 먼저 일시정지·저장한 뒤 선택한 Job을 복원하여 두 대화방 상태 재확인부터 이어갑니다.

새 Job 시작 시 다른 비종료 Job이 실제 실행 중이면 시작하지 않고 해당 Job ID와 함께 먼저 `일시정지` 또는 `중지`하라는 확인창을 표시합니다. 기존 Job이 이미 수동 일시정지되어 실행 슬롯을 양보했다면 별도 차단 없이 새 Job 하나를 시작합니다.
