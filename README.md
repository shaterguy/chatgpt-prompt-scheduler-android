...
v0.1.15에서는 오토런 중계의 감시·수신·전달·제출 확인 상태를 분리해 저장하고, 정상 응답은 시간 제한 없이 감시합니다. 제출 전후 프로세스가 종료되어도 DOM의 동일 사용자 턴을 확인해 중복 클릭을 막으며, 불명확한 결과와 규정 외 응답은 알림 후 안전하게 일시정지합니다. `WAITING_USER` 및 사용자 조치 재검증 흐름을 추가했고, 기존 예약 생성·저장·알림·실행 경로와 JSON 형식은 변경하지 않았습니다.
