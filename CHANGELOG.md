...
- 검증된 `v0.5.1-dev1` 기능 상태를 기존 정식 앱 계보의 `0.5.1`로 승격
- 예약 프롬프트 전송 후 실제 대화 주소가 생성되었는지, 대상이 일반 Chat·프로젝트·기존 대화 중 기대한 컨텍스트인지, 전송한 사용자 프롬프트가 실제 대화에 존재하는지를 함께 검증
- 일반 Chat 홈이나 프로젝트 루트처럼 실제 `/c/...` 대화 주소가 아직 생성되지 않은 상태를 완료로 조기 판정하지 않고 계속 확인
- 검증된 실제 대화 주소를 실행 기록의 `conversationUrl`에 저장하고 기존 schema 2 실행 기록도 계속 읽을 수 있도록 호환성 유지
- `v0.5.1-dev1` SHA `5bccb643cce79f927b97270d370309e2758d2c72`에서 단위 테스트 68건과 Android/WebView 테스트 20건, 총 88건의 기존 독립 검증 PASS를 기능 증거로 재사용
- 정식 Application ID와 공개 서명 계보를 유지하고 `versionCode 2100000006`으로 최신 정식 `v0.5.0`에서의 업데이트 경로를 검증하도록 릴리스 구성
